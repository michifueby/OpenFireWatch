...
| A status page that reports per feed, not per process | The health endpoint answers "is the process up" and the ingestion watchdog "did a cycle run". Neither could answer "what did we ask, and did anyone reply" — and this deployment was healthy by both while watching one satellite in three. Every feed now reports when it last delivered, and `missing` is a different word from `stale` | One more read that touches Redis, the database and the dead-letter queue on each call |
| A past day on the map is a mode, not a filter | Showing a chosen day alongside the live week would answer a question nobody asked, and leaving the pulsing markers up would claim a fire from 2021 still needs somebody. The markers come off, the camera stops chasing, and a banner says what is on screen | Live detections arriving while a past day is shown are dropped from the view; the live set is refetched on return |
| A zone may carry two hazards, with separate gates | The Föhrenwald is a pine forest AND contaminated with buried white phosphorus. Tying "which ignition window applies" to "what escalates" forced a choice: the phosphorus gate would have left a real fire on a cool April day at ELEVATED, paging nobody. `tracksIgnitionWindow` is now separate from `requiresIgnitionWeather`, so such a zone alarms on any credible detection *and* still measures and forecasts the window the ground sensors exist to improve | One more field on every hazard profile, and a readiness line that says two things instead of one |
| All three VIIRS instruments polled, not one | They fly the same orbit about forty minutes apart, so each is a separate look at the ground. On 14 August 2026 over the Föhrenwald, NOAA-20 saw the fire at 10:41, NOAA-21 at 11:25 and Suomi NPP at 12:00 — polling only Suomi NPP costs 79 minutes and three quarters of the observations | Three FIRMS transactions per cycle instead of one, against a budget of 5000 per ten minutes |
| A two-day poll window, not one | Near-real-time is not punctual: a pass published more than 24 h after acquisition would be missed for good, because the next cycle asks about a window it has already left. Re-asking yesterday is free — job ids and the unique constraint are both keyed on (source, pixel, acquisition time) | Each cycle offers passes it has already seen; the counts in the log are what was offered, not what was new |
